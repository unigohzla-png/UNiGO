...
class RegisterCoursesController extends ChangeNotifier {
  ...
  /// Only courses the student is allowed to register now:
  /// - same facultyId
  ...
  Future<void> loadInitialData() async {
    ...
    try {
      ...
      // ----- Read ONLY available next semester courses in same faculty -----
      ...
      for (final doc in coursesSnap.docs) {
        ...
        // If the user already has it in upcomingCourses, attach selected section (if exists)
  ...
  Subject _subjectFromCourseDoc(
    ...
    // parse sections (stored as array of maps in the course doc)
    List<CourseSection> sections = [];
    final rawSections = data['sections'];
    if (rawSections is List) {
      sections = rawSections
